...
function themeClasses(theme) {
  return theme === "dark"
    ? {
        page: "bg-stone-950 text-white",
        topBar: "border-white/10 bg-stone-950/90",
        header: "border-white/10 bg-stone-950/85",
        card: "border-white/10 bg-white/5 text-white",
        softCard: "border-white/10 bg-stone-900/70 text-white",
        input: "border-white/10 bg-black/20 text-white placeholder:text-white/35",
        muted: "text-white/70",
        subtle: "text-white/60",
        faint: "text-white/45",
        footer: "border-white/10 bg-stone-950/95 text-white/60",
        avatar: "bg-white/10 text-white/45",
      }
    : {
        page: "bg-stone-100 text-stone-900",
        topBar: "border-stone-300/70 bg-white/90",
        header: "border-stone-300/70 bg-white/90",
        card: "border-stone-300/70 bg-white text-stone-900",
        softCard: "border-stone-300/70 bg-white text-stone-900",
        input: "border-stone-300/70 bg-white text-stone-900 placeholder:text-stone-400",
        muted: "text-stone-700",
        subtle: "text-stone-600",
        faint: "text-stone-500",
        footer: "border-stone-300/70 bg-white/95 text-stone-600",
...
export default function CASWebsite() {
            ...
            <>
              ...
              <form onSubmit={handlePublishUpdate} className="mt-8 space-y-5">
                ...
                <button
                  ...
                  className={theme === "dark" ? "w-full rounded-2xl border border-white/15 bg-white/5 px-5 py-3 font-semibold text-white transition hover:bg-white/10" : "w-full rounded-2xl border border-stone-300/70 bg-white px-5 py-3 font-semibold text-stone-900 transition hover:bg-stone-100"}
                >
                  Reset Updates
                </button>
              </form>
              ...
              <div className={theme === "dark" ? "mt-10 border-t border-white/10 pt-8" : "mt-10 border-t border-stone-300/70 pt-8"}>
                <div>
                  ...
                  <p className={`mt-2 ${t.subtle}`}>
                    Add new team members, update roles, and paste image links that will appear in the public leadership section.
                  </p>
                </div>
                ...
                >
                  ...
                  <button
                    ...
                    className={theme === "dark" ? "mt-3 w-full rounded-2xl border border-white/15 bg-white/5 px-5 py-3 font-semibold text-white transition hover:bg-white/10" : "mt-3 w-full rounded-2xl border border-stone-300/70 bg-white px-5 py-3 font-semibold text-stone-900 transition hover:bg-stone-100"}
                  >
  ...
  const studentPanel = (
    <div className={`min-h-screen ${t.page}`}>
      <TopBar
        ...
        setTheme={setTheme}
      />
      <div className="mx-auto grid min-h-[calc(100vh-80px)] max-w-7xl gap-10 px-4 py-12 sm:px-6 sm:py-16 lg:grid-cols-[1fr_0.95fr] lg:px-8">
        <div className="flex flex-col justify-center">
          <p className="text-sm font-semibold uppercase tracking-[0.3em] text-emerald-300">
            Student Portal
          </p>
          <h1 className={theme === "dark" ? "mt-4 text-4xl font-bold leading-tight text-slate-100 transition duration-300 hover:-translate-y-0.5 hover:text-emerald-100 sm:text-5xl" : "mt-4 text-4xl font-bold leading-tight text-stone-900 transition duration-300 hover:-translate-y-0.5 hover:text-emerald-700 sm:text-5xl"}>
            A dedicated space for CAS students.
          </h1>
          <p className={`mt-6 max-w-2xl text-lg leading-8 ${t.muted}`}>
            Students can access event information, leadership updates, and future member-only resources in one clean portal.
          </p>
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            <PortalFeature
              title="Upcoming events"
              text="View current CAS events and campus updates in a focused student experience."
              theme={theme}
            />
            <PortalFeature
              title="Member information"
              text="A home for future signups, announcements, and community resources."
              theme={theme}
            />
          </div>
        </div>
        ...
        <div className={`rounded-[2rem] border p-6 shadow-2xl backdrop-blur sm:p-8 ${t.card}`}>
          {!studentLoggedIn ? (
            <>
              ...
              <p className={`mt-3 ${t.muted}`}>
                This is the front-end portal design. Real student authentication can be connected before deployment.
              </p>
              <form onSubmit={handleStudentLogin} className="mt-8 space-y-5">
                <InputField
                  label="CMC Email"
                  type="email"
                  value={studentForm.email}
                  onChange={(value) => setStudentForm({ ...studentForm, email: value })}
                  placeholder="student@cmc.edu"
                  theme={theme}
                />
                <InputField
                  label="Password"
            ...
            <div>
              <div className="flex items-center justify-between gap-4">
                ...
                <button
                  type="button"
                  onClick={() => setStudentLoggedIn(false)}
                  className={theme === "dark" ? "rounded-xl border border-white/15 px-4 py-2 text-sm text-white/80" : "rounded-xl border border-stone-300/70 px-4 py-2 text-sm text-stone-700"}
                >
                  Log out
                </button>
              ...
              <div className="mt-8 space-y-4">
                {updates.map((item) => (
                  <div
                    key={item.id}
                    className={theme === "dark" ? "rounded-2xl border border-white/10 bg-black/20 p-5" : "rounded-2xl border border-stone-300/70 bg-stone-50 p-5"}
                  >
                    <p className="text-xs uppercase tracking-[0.22em] text-emerald-300">
                      {item.category}
                    </p>
                    <h3 className={theme === "dark" ? "mt-2 text-2xl font-semibold text-white" : "mt-2 text-2xl font-semibold text-stone-900"}>
                      {item.title}
                    </h3>
                    <p className={`mt-2 text-sm ${t.faint}`}>{item.date}</p>
                    <p className={`mt-4 ${t.muted}`}>{item.description}</p>
                  </div>
                ))}
              </div>
            </div>
  ...
  if (view === "student") return studentPanel;
        ...
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button
              type="button"
              onClick={() => setView("student")}
              className={theme === "dark" ? "rounded-2xl border border-white/15 bg-white/5 px-4 py-2 text-sm font-semibold text-white" : "rounded-2xl border border-stone-300/70 bg-white px-4 py-2 text-sm font-semibold text-stone-900"}
            >
              Student Portal
            </button>
            <button
        ...
        <section className="relative overflow-hidden">
          ...
          <div className="mx-auto grid max-w-7xl gap-12 px-4 py-14 sm:px-6 sm:py-20 lg:grid-cols-2 lg:px-8 lg:py-24">
            <div className="relative z-10 flex flex-col justify-center">
              ...
              <div className="mt-8 flex flex-col gap-4 sm:flex-row sm:flex-wrap">
                <button
                  ...
                  className="rounded-2xl bg-white px-5 py-3 font-semibold text-stone-950"
                >
                  See Latest Updates
                </button>
                <button
                  type="button"
                  onClick={() => setView("student")}
                  className={theme === "dark" ? "rounded-2xl border border-white/15 bg-white/5 px-5 py-3 font-semibold text-white" : "rounded-2xl border border-stone-300/70 bg-white px-5 py-3 font-semibold text-stone-900"}
                >
                  Open Student Portal
                </button>
              </div>
            ...
            <div className="relative z-10">
              <img
                src={FAVICON_URL}
                alt="CMC students smiling"
                className="h-[260px] w-full rounded-[2rem] object-cover shadow-2xl sm:h-[340px] lg:h-[430px]"
              />
